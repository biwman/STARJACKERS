...
public sealed class ScienceStationSpawner : MonoBehaviourPunCallbacks
{
    public const string LayoutKey = "scienceStationLayout";
    ...
    const float MinDistanceBetweenStations = 11f;
    const float MinDistanceFromExtractionZones = 14f;
    const float MinDistanceFromRepairBays = 10f;
    const float MinDistanceFromFactories = 11f;
    const float MinDistanceFromObstacles = 7f;
    const float MinDistanceFromNebulas = 5.8f;
    const float StrictOverlapRadius = 3.25f;
    const float RelaxedOverlapRadius = 1.2f;

    static ScienceStationSpawner instance;
    ...
    public static void EnsureExists()
    {
        ...
        GameObject existing = GameObject.Find("ScienceStationSpawner");
        if (existing != null && existing.TryGetComponent(out ScienceStationSpawner existingSpawner))
        {
            instance = existingSpawner;
            return;
        }

        GameObject spawner = new GameObject("ScienceStationSpawner");
        instance = spawner.AddComponent<ScienceStationSpawner>();
    }
    ...
    void ResetLocalRuntimeState()
    {
        StopAllCoroutines();
        layoutApplied = false;
        ScienceStation.ClearAllRuntimeStations();
        StartCoroutine(InitializeWhenRoundStarts());
    }
    ...
    string BuildLayout(int seed, List<Vector2> extractionPositions, List<Vector2> repairBayPositions, List<Vector2> factoryPositions, List<Vector2> obstaclePositions, List<Vector2> nebulaPositions)
    {
        int count = RoomSettings.GetScienceStationCount();
        if (count <= 0)
        ...
        if (positions.Count < count)
        {
            FillPositions(
                ...
                nebulaPositions,
                MinDistanceFromExtractionZones,
                MinDistanceFromRepairBays * 0.65f,
                MinDistanceFromFactories * 0.65f,
        ...
        for (int i = 0; i < positions.Count; i++)
        {
            ...
            float phase = Mathf.Repeat(Mathf.Sin((seed * 23.41f) + (i * 73.7f)) * 1000f, Mathf.PI * 2f);
            builder.Append("science_station_");
            builder.Append(i);
            builder.Append(',');
        ...
        int maxAttempts)
    {
        ...
        while (positions.Count < count && attempts < maxAttempts)
        {
            ...
            if (IsTooCloseToAny(pos, positions, MinDistanceBetweenStations))
                continue;
            ...
            if (IsTooCloseToAny(pos, factoryPositions, factoryDistance))
                continue;
    ...
    void ApplyLayout(string layout)
    {
        ...
        for (int i = 0; i < entries.Length; i++)
        {
            ...
            GameObject stationObject = new GameObject("ScienceStation_" + id, typeof(SpriteRenderer), typeof(CircleCollider2D));
            ScienceStation station = stationObject.AddComponent<ScienceStation>();
            station.Configure(id, new Vector2(x, y), phase);
        }
    }
    ...
    int ResolveSeed()
    {
        ...
        seed = CombineSeed(seed, RoomSettings.GetScienceStationCount());
